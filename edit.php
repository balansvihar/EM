...
<form action="list.php" method="post" onSubmit=" return validate()">
	<label for="date">Date</label><input type="date" name="date" id="date" value="<?php echo  $expense->formatDate( $exp['datee']);?> "/>
    <label for="type">Type</label>
    <select name="type">
    	<option value="expense" <?php if($exp['type']=="expense") echo 'selected';?>  >Expense</option>
        <option value="income" <?php if($exp['type']=="income") echo 'selected';?> >Income</option>
     </select>
	<label for="payee">Payee</label><input name="payee" type="text"  id="payee" value="<?php echo $exp['payee'];?>"/>
	<label for="amount">Amount</label><input type="tel" name="amount" id="amount" value="<?php echo $exp['amount'];?>"/>
    <input type="submit" name="save" id="save" value="SAVE">

  <a href="list.php?month=<?php echo $_GET['month'];?>&page=<?php echo $_GET['page'];?>"><input type="button" name="list" id="list" value="LIST"></a>
      <input type="hidden" name="id" id="id" value="<?php echo$_GET['idx'];?>">
</form>
